...
static char base36Map[36]; //0..10 a..z
...
static char* GenerateRandomEightCharacters(char *randomString)
{
	...
	{
		int rawRandom = rand();
		int first = rawRandom % 36;
		int second = rawRandom / 36 % 36;
...
static void AddDiagnosticsPropertiesIfNecessary(IOTHUB_MESSAGE_HANDLE eventMessageHandle)
{
	...
	if (floor((sequenceNumberOfMessage - 2) * diagSamplingRate / 100) < floor((sequenceNumberOfMessage - 1) * diagSamplingRate / 100))
	{
		char randomId[9];
		char timeBuffer[21];
...
static void DiagnosticsDeviceTwinCallBack(DEVICE_TWIN_UPDATE_STATE update_state, const unsigned char* deviceTwinJson, size_t size, void* userContextCallback)
{
	...
	jsmn_init(&parser);
	parsedTokenCount = jsmn_parse(&parser, payLoad, size, tokens, 100);
	for (int index = 0; index < parsedTokenCount; ++index)
	{
		...
		{
			isServerDiagnosticEnabled = tokens[index+1].type == JSMN_STRING && _strnicmp(payLoad + tokens[index+1].start, "true", 4) == 0;
